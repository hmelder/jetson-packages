#!/usr/bin/env bash
# Description: Build the deb packages from the source packages (source/t210/debian-desc)
...
function makeSourceTarball {
    ...
    local sourceDir=${SOURCE_T210_DIR}/${pkgName}
    ...
    tar -czf ${SOURCE_DEB_DIR}/${tarballName} -C ${sourceDir} .
    if test $? -ne 0; then
...
function findSourcePackage {
    ...
    for entry in "${SOURCE_PKG_ARRAY[@]}"; do
        if test "$(basename "$entry")" == "${PACKAGE_NAME}"; then
            ...
            if ! buildSourcePackage "${SOURCE_T210_DIR}/${phase}" "${PBUILDER_RESULT}" "${phase}"; then
